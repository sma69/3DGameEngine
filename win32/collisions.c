...
int sphere_sphere_intersection(
	Vector3D c1, float r1,   /*sphere 1*/
	Vector3D c2, float r2    /*sphere 2*/
)
{
	float distance;
	distance =
...
int point_cube_intersection(
	...
	Vector3D box, Vector3D size)  /*cube*/
{
	if ((point.x < box.x) ||
		(point.y < box.y) ||
		(point.z < box.z) ||
...
int sphere_cube_intersection(
	Vector3D center, float r,   /*sphere*/
	Vector3D box, Vector3D size)  /*axis aligned 3d cube*/
{
	float distance;
	Vector3D offset;
